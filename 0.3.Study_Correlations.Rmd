...
```{r}
tec2 <-  tec[ ,!names(tec) %in% c("SampleID","Case")]

names(tec2) <- c("INDEX.i7","INDEX.i5","FC","LANE","LIBRARY","RIN","DV200","QUBIT","GC_MEAN")
...
```{r}
ggboxplot(subset(bio2, !is.na(TYPE)), x = "TYPE", y = "DIAMETER") +
  geom_boxplot() +
...
```{r}
ggboxplot(subset(bio2, !is.na(SYMPTOMS)), x = "SYMPTOMS", y = "DIAMETER") +
  geom_boxplot() +
  stat_compare_means(method = "t.test", label.y = 110) +
  scale_x_discrete(breaks = c(0,1), labels = c("Asymptomatic","Symptomatic"))
```
...
```{r, warning=FALSE}
ggscatter(bio2, x = "WEIGHT", y = "HEIGHT",
          add = "reg.line") +
  stat_cor(method = "pearson")
```
...
filter.cases <-  all.cases[ ,c("SEX","AGE","TYPE","SYMPTOMS","HEIGHT","WEIGHT","SMOKING","O.ANEURYSM","DIAMETER")]
```
...
```{r, warning=FALSE}
ggscatter(b.cases, x = "AGE", y = "WEIGHT",
          add = "reg.line") +
  stat_cor(method = "pearson")
```
...
```{r}
tab <- as.data.frame(prop.table(table(b.cases$SYMPTOMS, b.cases$TYPE)))
...
```{r}
ggboxplot(subset(b.cases, !is.na(TYPE)), x = "TYPE", y = "DIAMETER") +
  geom_boxplot() +
  stat_compare_means(method = "t.test", label.y = 110) +
  scale_x_discrete(breaks = c(1,2), labels = c("Atherosclerotic","Inflammatory"))
```
...
```{r}
tab <- as.data.frame(prop.table(table(b.cases$SYMPTOMS, b.cases$SMOKING)))
...
```{r}
ggboxplot(subset(b.cases, !is.na(SYMPTOMS)), x = "SYMPTOMS", y = "DIAMETER") +
  geom_boxplot() +
  stat_compare_means(method = "t.test", label.y = 110) +
  scale_x_discrete(breaks = c(0,1), labels = c("Asymptomatic","Symptomatic"))
```
...
```{r, warning=FALSE}
ggscatter(b.cases, x = "WEIGHT", y = "HEIGHT",
          add = "reg.line") +
  stat_cor(method = "pearson")
```
...
```{r}
# Remove columns that have no variation:
all.controls <-  all.controls[ ,!names(all.controls) %in% c("STATUS","TYPE","SYMPTOMS","HEIGHT","WEIGHT","O.ANEURYSM","DIAMETER")]
```
...
png(file = "C://Users/Gerard/Desktop/AAA/RNAseq/Plots/Correlations/Correlation_Plot_Controls.png")
corrplot(M, p.mat = testRes$p, sig.level = 0.05, type = "lower", insig = "blank", tl.cex = 2)
dev.off()
