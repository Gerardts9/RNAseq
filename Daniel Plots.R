...
design <-  model.matrix(~ AAA + tec$FC + tec$Lane + tec$GC_Mean + tec$RIN + tec$DV200 + tec$Qubit + bio$SEXO + bio$age)
...
##################################################
ann <- fread("/home/gerard/AAA/refs/gencode.v26.annotation.fixed.gtf.gz")
counts.tpm <- read.table("/home/gerard/AAA/Gene_Counts/Counts.RSEM.Cases.txt")
tpm_matrix <- as.matrix(counts.tpm)
...
for (gene in glist) {
  res <-
    resid(
      lm(
        counts[gene, ] ~ tec2$Date + tec2$Batch + tec2$GC_Mean + tec2$RIN + tec2$DV200 + tec2$Qubit + bio2$age
      )
    )
  ...
  text <-
    format(as.numeric(Results[rownames(Results) %in% gene, ]$fdr), digits = 2)
  
  ggplot(res, aes(x = aortc_diameter_mm, y = Expression)) + geom_point(color = "orangered2") +
    ggtitle(gene) +
    ylab ("Expression") +
    xlab("Aortic Diameter (mm)") +
    ...
    annotate(
      ...
      x = x_coord,
      y = y_coord,
      label = paste0("FDR P-value:", text)
    )
  
  ggsave(paste0("/home/gerard/", gene, ".png"),
         width = 7,
         height = 5)
}
