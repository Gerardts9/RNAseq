...
design <-
  model.matrix(~ AAA + tec$FC + tec$Lane + tec$Library + tec$GC_Mean + tec$RIN + tec$DV200 + tec$Qubit + bio$SEXO + bio$age + bio$Smoking)
...
Pvalue <-
  apply(counts.tpm, 1, function(x)
    summary(
      lm(
        x ~ tec$Case + tec$FC + tec$Lane + tec$Library + tec$GC_Mean + tec$RIN + tec$DV200 + tec$Qubit + bio$age + bio$SEXO + bio$Smoking
      )
    )[4][[1]][2, 4])
Coef <-
  apply(counts.tpm, 1, function(x)
...
sig.genes <- rownames(Results[Results$fdr < 0.05,])
sig.genes <- ann[ann$gene_name %in% sig.genes,]$gene_id

#write.table(sig.genes, "C://Users/Gerard/Desktop/AAA/RNAseq/Gene_counts/Significant_genes.txt", quote = F, row.names = F, col.names = F)

#write.table(sig.genes, "C://Users/Gerard/Desktop/AAA/RNAseq/Gene_counts/Significant_genes_smoking.txt", quote = F, row.names = F, col.names = F)
