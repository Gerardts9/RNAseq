...
# Aortic Diameter:
bio$aortc_diameter_mm_2 <- ifelse(bio$aortc_diameter_mm > 55, 2, 1)
table(bio$aortc_diameter_mm_2, useNA = "always")
```


```{r}
bio2 <- na.omit(bio[,c("Muestra","aortc_diameter_mm_2")])

ggplot(bio2, aes(x=aortc_diameter_mm_2, y = Muestra)) + 
  geom_point()
```
...
### Manual Differential Expression (No smoking):
...
### Manual Differential Expression (Including Smoking) (Sensibility analysis):
...
Pvalue <-
  apply(counts.tpm, 1, function(x)
    summary(
      lm(
        x ~ bio$aortc_diameter_mm + tec$Date + tec$Batch + tec$GC_Mean + tec$RIN + tec$DV200 + tec$Qubit + bio$age + bio$SEXO + bio$Smoking
      )
    )[4][[1]][2, 4])
Coef <-
  apply(counts.tpm, 1, function(x)
...
# 32 DEGs when not correcting for Smoking.
# 19 DEGs when also correcting for Smoking.
```
...
intersect(cc$Gene.Name, rownames(Results[Results$fdr < 0.05,]))
# EXTL3, ZFR, DUSP8, RFX1.

setdiff(rownames(Results[Results$fdr < 0.05,]), cc$Gene.Name)

length(intersect(cc$Gene.Name, rownames(Results[Results$fdr < 0.05,])))
...
### Sensitivity results comparing with Large vs Control and Small vs Control:
```{r}
Results[Results$fdr < 0.05,]
sig.dia <- rownames(Results[Results$fdr < 0.05,])
sig.dia # 32 Genes.

sm <- read_excel("C://Users/Gerard/Downloads/Small_AAA.xls")
lar <- read_excel("C://Users/Gerard/Downloads/Large_AAA.xls")

sm[sm$Symbol %in% sig.dia,]
lar[lar$Symbol %in% sig.dia,]


sig.res <- read.xlsx("C://Users/Gerard/Desktop/AAA/RNAseq/SigRes/Cases_Controls_Confounders.xlsx")
nrow(sig.res) # 7,454


sig.res[sig.res$Gene.Name %in% sig.dia,] # 4 Common between Diameter and Case Control CONFOUNDERS.

Results[rownames(Results) %in% c("USP33"),] # Beta > 0.

Results[rownames(Results) %in% c("EXTL3","ZFR","VPS37C"),]
```
