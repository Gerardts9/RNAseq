...
#write.xlsx(Results[Results$fdr < 0.05,], "C://Users/Gerard/Desktop/AAA/RNAseq/SigRes/Diameter.xlsx", sep = "\t", row.names = T)
```
...
```{r}
res <- resid(lm(counts["EGFR",] ~ tec2$Date + tec2$Batch + tec2$GC_Mean + tec2$RIN + tec2$DV200 + tec2$Qubit + bio2$age))
...
```{r}
p <- ggplot(res, aes(x = gg, y = Expression, fill = gg)) + geom_boxplot() + ggtitle("EGFR") + ylab("Expression") +
  theme(
...
#ggsave("C://Users/Gerard/Desktop/AAA/RNAseq/Plots/ERBB2_Diameter.png")
```
...
hist(Pvalue, main = "Symptoms", cex.main = 2)
dev.off()
...
#write.xlsx(Results[Results$fdr < 0.05,], "C://Users/Gerard/Desktop/AAA/RNAseq/SigRes/Symptoms.xlsx", sep = "\t", row.names = T)
```
