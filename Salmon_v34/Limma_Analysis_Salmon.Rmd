...
design <- model.matrix(~AAA+tec$Date+tec$GC_Mean+tec$RIN)

colnames(design) <- c("(Intercept)", "AAACase", "Date202220829", "GC_Mean", "RIN")
...
```{r}
lm.deg <- topTable(fit, coef=1, number = Inf)
