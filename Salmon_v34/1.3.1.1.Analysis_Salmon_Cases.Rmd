...
```{r}
counts.tpm <- fread("C://Users/Gerard/Desktop/AAA/RNAseq/Gene_counts/Counts.Salmon.V34_Exh.txt")
...
design <-
  model.matrix(
    ~ bio$aortc_diameter_mm + tec$Date + tec$GC_Mean + tec$Batch + tec$RIN)
...
paste0("We keep ", sum(keep), " genes") # 10,222
paste0("We remove ", sum(!keep), " genes") # 26,501
...
Pvalue <-
  apply(cases.tpm, 1, function(x)
    summary(
      lm(
        x ~ bio$aortc_diameter_mm + tec$Date + tec$GC_Mean + tec$Batch + tec$RIN
      )
    )[4][[1]][2, 4])
...
#write.xlsx(Results.Salmon.V34[order(Results.Salmon.V34$pvalue), ][1:23,], "C://Users/Gerard/Desktop/AAA/RNAseq/GitHub/TopTable_Diameter.xlsx", rowNames = TRUE)
nrow(Results.Salmon.V34[Results.Salmon.V34$fdr < 0.05,]) # Exh: 45 Exh_0: 15.

rownames(Results.Salmon.V34[Results.Salmon.V34$fdr < 0.05,])

#fwrite(Results.Salmon.V34, "C://Users/Gerard/Desktop/AAA/RNAseq/DEG_Diameter/Salmon.V34_Exh_0.txt", sep = "\t", row.names = T)
```
