...
library(data.table);library(edgeR);library(tidyverse);library(openxlsx);library(preprocessCore);library(qvalue);library(clusterProfiler)
library(stats)
...
design <-
  model.matrix(
    ~ bio$aortc_diameter_mm + tec$Date + tec$GC_Mean + tec$Batch + tec$RIN + bio$age + bio$Smoking + bio$other_aneurysm   )
...
Pvalue <-
  apply(dge$counts, 1, function(x)
    summary(
      lm(
        x ~ bio$aortc_diameter_mm + tec$Date + tec$GC_Mean + tec$Batch + tec$RIN + bio$age + bio$Smoking + bio$other_aneurysm
      )
    )[4][[1]][2, 4])
...
Results.size <-
  merge(Results.size, ann[, c("gene_id", "gene_name")], by.x = 0, by.y = "gene_id")
names(Results.size)[names(Results.size) == "Row.names"] <- "gene_ID"
Results.size <- Results.size[!duplicated(Results.size$gene_name),]
Results.size <-
  Results.size %>% remove_rownames %>% column_to_rownames(var = "gene_ID")

#write.xlsx(Results.size[order(Results.size$pvalue), ][1:76,], "C://Users/Gerard/Desktop/AAA/RNAseq/GitHub/TopTable_Diameter.xlsx", rowNames = TRUE)
nrow(Results.size[Results.size$fdr < 0.05,])
```
...
design <-
  model.matrix(
    ~ bio$TIPOANEURISMA + tec$Date + tec$GC_Mean + tec$Batch + tec$RIN + bio$age + bio$Smoking + bio$other_aneurysm
  )
...
Pvalue <-
  apply(dge$counts, 1, function(x)
    summary(
      lm(
        x ~ bio$TIPOANEURISMA + tec$Date + tec$GC_Mean + tec$Batch + tec$RIN + bio$Smoking + bio$other_aneurysm
      )
    )[4][[1]][2, 4])
Coef <-
  apply(dge$counts, 1, function(x)
